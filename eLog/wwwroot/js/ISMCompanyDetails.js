...
function sortTable(columnIndex) {
    var table = document.getElementById("ismCompanyTable");
...
function filterTable() {
    ...
    var rows = document.querySelectorAll("#ismCompanyTable tbody tr");
...
function changePage(offset) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        var basePath = document.querySelector('base')?.getAttribute('href') || '/';
        window.location.href = `${basePath}/ISMCompanyDetails/GetISMCompanyDetails?pageNumber=${newPage}&pageSize=${pageSize}`;
    }
}