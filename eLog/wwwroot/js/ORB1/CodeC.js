...
function openAddWindow() {
    ...
    var url = `${basePath}/CodeC/DataEntry_CodeC?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function sortTable(columnIndex) {
    var table = document.getElementById("codeCTable");
...
function filterTable() {
    ...
    var rows = document.querySelectorAll("#codeCTable tbody tr");
...
function editRecord(id) {
    ...
    // Encode the values to handle spaces and special characters
    var url = `${basePath}/CodeC/Edit/${id}?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function changePage(offset) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        var basePath = document.querySelector('base')?.getAttribute('href') || '/';
        window.location.href = `${basePath}/ORB1/CodeC/GetCodeCData?pageNumber=${newPage}&pageSize=${pageSize}`;
    }
}