...
function openAddWindow() {
    ...
    var url = `${basePath}/CodeF/DataEntry_CodeF?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function sortTable(columnIndex) {
    var table = document.getElementById("codeFTable");
...
function filterTable() {
    ...
    var rows = document.querySelectorAll("#codeFTable tbody tr");
...
function editRecord(id) {
    ...
    // Encode the values to handle spaces and special characters
    var url = `${basePath}/CodeF/Edit/${id}?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function changePage(offset) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        var basePath = document.querySelector('base')?.getAttribute('href') || '/';
        window.location.href = `${basePath}/ORB1/CodeF/GetCodeFData?pageNumber=${newPage}&pageSize=${pageSize}`;
    }
}