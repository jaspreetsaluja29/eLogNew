...
function openAddWindow() {
    ...
    var url = `${basePath}/CodeD/DataEntry_CodeD?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function sortTable(columnIndex) {
    var table = document.getElementById("codeDTable");
...
function filterTable() {
    ...
    var rows = document.querySelectorAll("#codeDTable tbody tr");
...
function editRecord(id) {
    ...
    // Encode the values to handle spaces and special characters
    var url = `${basePath}/CodeD/Edit/${id}?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function changePage(offset) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        var basePath = document.querySelector('base')?.getAttribute('href') || '/';
        window.location.href = `${basePath}/ORB1/CodeD/GetCodeDData?pageNumber=${newPage}&pageSize=${pageSize}`;
    }
}