...
function openAddWindow() {
    ...
    var url = `${basePath}/CodeE/DataEntry_CodeE?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function sortTable(columnIndex) {
    var table = document.getElementById("codeETable");
...
function filterTable() {
    ...
    var rows = document.querySelectorAll("#codeETable tbody tr");
...
function editRecord(id) {
    ...
    // Encode the values to handle spaces and special characters
    var url = `${basePath}/CodeE/Edit/${id}?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function changePage(offset) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        var basePath = document.querySelector('base')?.getAttribute('href') || '/';
        window.location.href = `${basePath}/ORB1/CodeE/GetCodeEData?pageNumber=${newPage}&pageSize=${pageSize}`;
    }
}