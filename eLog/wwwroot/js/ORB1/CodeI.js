...
function openAddWindow() {
    ...
    var url = `${basePath}/CodeI/DataEntry_CodeI?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function sortTable(columnIndex) {
    var table = document.getElementById("codeITable");
...
function filterTable() {
    ...
    var rows = document.querySelectorAll("#codeITable tbody tr");
...
function editRecord(id) {
    ...
    // Encode the values to handle spaces and special characters
    var url = `${basePath}/CodeI/Edit/${id}?userId=${encodeURIComponent(userId)}&userName=${encodeURIComponent(userName)}&userRoleName=${encodeURIComponent(userRoleName)}&jobRank=${encodeURIComponent(jobRank)}`;
...
function changePage(offset) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        var basePath = document.querySelector('base')?.getAttribute('href') || '/';
        window.location.href = `${basePath}/ORB1/CodeI/GetCodeIData?pageNumber=${newPage}&pageSize=${pageSize}`;
    }
}